...
def getnrecentinfo(repo, timestamps, info, nrecent=None):
    ...
    for commit in commits:
        ...
        if dt > timestamps["today-7d"]:
            info["ncommits_last_7d"] += 1
        elif dt > timestamps["today-30d"]:
            info["ncommits_last_30d"] += 1
        elif dt > timestamps["today-6m"]:
            info["ncommits_last_6m"] += 1
        else:
            info["ncommits_last_1y"] += 1
        if author not in info["ncommits_authors"].keys():
            info["ncommits_authors"][author] = 1
        else:
            info["ncommits_authors"][author] += 1
    return info
...
def getreposummary(name, timestamps, nrecent):
    ...
    repo = git.Repo(
        name
    )  # Need to be in the directory with the repo or pass the Path to it
    ...
    if summary["agelast"].days < 30:
        summary = getnrecentinfo(
            repo=repo, timestamps=timestamps, info=summary, nrecent=nrecent
        )
    return summary
...
def format_ncommitinfo(row):
    ncommits = f"N commits (last 7d | 30d | 6m | 1y): {row.ncommits_last_7d:.0f} | {
        row.ncommits_last_30d:.0f} | {row.ncommits_last_6m:.0f} | {
        row.ncommits_last_1y:.0f}"
    return ncommits
...
def unpacktext(proctext):
    ...
    for iss in issues:
        ...
        if status == "OPEN":
            nopen += 1
            year = dt[:4]
            if year not in opened_years.keys():
                opened_years[year] = 1
            else:
                opened_years[year] += 1
...
def formatrecent(df):
    ...
    copy["stats"] = copy[["visibility", "nwatchers", "nstargazers"]].apply(
        lambda row: (
            f"{row.visibility.title()} | {row.nwatchers} 👀 | {row.nstargazers} ⭐️"
        ),
        axis=1,
    )
    copy.languages = copy.languages.apply(formatlanguages)
    copy["lifespan"] = copy[["year_created", "nyears_active"]].apply(
        lambda row: (
            f"Created: {row.year_created} | Years Active: {row.nyears_active}"
            if row.nyears_active > 0
            else f"Created: {row.year_created} | Years Active: Less than 1"
        ),
        axis=1,
    )
