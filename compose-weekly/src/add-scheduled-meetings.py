...
def format_dtstart(event, local="America/Los_Angeles"):
    ...
    if not isinstance(dtdt_start, datetime):
        return dtdt_start
    if pd.isna(dtdt_start.tzinfo):
        return dtdt_start
...
def get_event_info(event, dtstart):
    ...
    if not isinstance(dtstart, datetime):
        ...
        time = dtstart.time().strftime("%H:%M")
    return {
...
def find_events(ecal, caldate):
    ...
    for event in recurring_ical_events.of(ecal).at(caldate):
        ...
        if isinstance(dtstart, datetime):
            if (dtstart.hour < 9) | (dtstart.hour > 17):
                continue
        if dtstart.isoweekday() > 5:
            continue
        if "SUMMARY" in event:
            if any(
                [
                    kw.lower() in str(event["SUMMARY"]).lower()
                    for kw in ("appt", "Office Hour", " OH")
                ]
            ):
                continue
            ...
            if "LOCATION" in event:
                ...
                if any(
                    [kw in info["location"].lower() for kw in ("zoom", "teams", "http")]
                ):
                    meet_type = "virtual"
                else:
                    meet_type = "in person"
            ...
            if (event["SUMMARY"].lower() == "payroll holiday") | (
                "birthday" in event["SUMMARY"].lower()
            ):
...
def get_events(ecal, caldate):
    ...
    for event in events:
        if not any(event["meet_type"]):
            ...
            text = f"{event['weekday']} {event['time']}: {event['title']} ({event['meet_type']})"
        out.append(text)
    return sorted(out)
...
def add_events(notes, events, prefix):
    ...
    if not any(events):
        return notes
    for text in events:
        notes.insert(prefix=prefix, text=text)
    return notes
