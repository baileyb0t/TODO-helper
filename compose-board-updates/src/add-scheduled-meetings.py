...
IGNOREABLE = (
    ...
    'appt', 'due',
    'office hour', ' oh',
    'time off', 'holiday', 'day off', 'moving', 'travel',
    'window',
    'inspection', 'diagnostic', 'car',
    'hana', 'ty', 'miranda',
)
...
def get_event_info(event, dtstart):
    ...
    else: time = dtstart.time().strftime('%I:%M%p').lower()
    return {
...
def find_events(ecal, caldate):
    ...
    for event in recurring_ical_events.of(ecal).at(caldate):
        ...
        if 'SUMMARY' in event:
            ...
            if event['SUMMARY'].lower() == 'payroll holiday':
...
def get_events(ecal, caldate):
    ...
    out = []
    for event in events:
        text = f"{event['date']} at {event['time']}: "
        if not any(event['meet_type']): text += event['title']
        else: text += f"{event['title']} ({event['meet_type']})"
        out.append(text)
    return sorted(out)


def add_events(notes, events, prefix):
    """Insert gather events into note."""
...
if __name__ == '__main__':
    ...
    e = date.today().replace(day=1) - relativedelta(days=1)
    s = (e - relativedelta(months=2)).replace(day=1)
    ndays = len(pd.date_range(s, e))
    logger.info(f'setting up meetings between {s} and {e}')

    notes.insert(prefix=formats['header'], text='On the calendar')
    notes.insert(prefix=formats['subheader'], text='Last quarter')
    for i in range(0, ndays):
        caldate = (s + relativedelta(days=+i)).strftime("%Y%m%d")
